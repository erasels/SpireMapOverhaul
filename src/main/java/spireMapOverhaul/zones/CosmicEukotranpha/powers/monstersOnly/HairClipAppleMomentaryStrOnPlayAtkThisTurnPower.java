...
public class HairClipAppleMomentaryStrOnPlayAtkThisTurnPower extends BasePower implements CloneablePowerInterface{
    public static final String POWER_ID=SpireAnniversary6Mod.makeID(HairClipAppleMomentaryStrOnPlayAtkThisTurnPower.class.getSimpleName());private static final PowerStrings powerStrings=CardCrawlGame.languagePack.getPowerStrings(POWER_ID);public static final String NAME=powerStrings.NAME;public static final String[]DESCRIPTIONS=powerStrings.DESCRIPTIONS;
    public int qqq=0;
    public HairClipAppleMomentaryStrOnPlayAtkThisTurnPower(AbstractCreature o,AbstractCreature s,int amount){super(POWER_ID);this.owner=o;this.source=s;
        this.canGoNegative=false;this.isTurnBased=true;this.type=PowerType.BUFF;this.amount=amount;updateDescription();}
    @Override public void onPlayCard(AbstractCard card,AbstractMonster m){if(card.type==AbstractCard.CardType.ATTACK){poO(new StrengthPower(owner,amount));poO(new LoseStrengthPower(owner,amount));}}
    ...
    @Override public AbstractPower makeCopy(){return new HairClipAppleMomentaryStrOnPlayAtkThisTurnPower(owner,source,amount);}}
