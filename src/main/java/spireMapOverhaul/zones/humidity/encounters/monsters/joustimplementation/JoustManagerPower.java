...
public class JoustManagerPower extends AbstractSMOPower implements InvisiblePower {
    ...
    public static boolean preJoustMonstersAreValid() {
        ...
        if (!(Wiz.getEnemies().get(0) instanceof HumidityCenturion
                && Wiz.getEnemies().get(1) instanceof HumidityHealer)) return false;
    ...
    public static boolean joustMonstersAreValid() {
        ...
        if (!(Wiz.getEnemies().get(0) instanceof HumidityCenturion
                && Wiz.getEnemies().get(1) instanceof HumidityCenturion
                && Wiz.getEnemies().get(2) instanceof HumidityHealer)) return false;
        for (AbstractMonster m : Wiz.getEnemies()) {
            if (m.isDying) return false;
        }
        return true;
