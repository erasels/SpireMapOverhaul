...
public class FlawedModifier extends AbstractManaSurgeModifier implements ModifierTags {
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        AbstractDungeon.actionManager.addToBottom(new ApplyPowerAction(AbstractDungeon.player, AbstractDungeon.player, new WeakPower(AbstractDungeon.player,WEAK_AMT,false),WEAK_AMT));
    }

    @Override
    public List<TooltipInfo> additionalTooltips(AbstractCard card) {
        List<TooltipInfo> tooltips = new ArrayList<>();
        tooltips.add(new TooltipInfo("[#ff8cd5]Flawed[]", "Gain #b1 #yWeak."));
        return tooltips;
    }

    @Override
    public String modifyDescription(String rawDescription, AbstractCard card) {
        return "[#ff8cd5]Flawed[]." + " NL " + rawDescription;
    }
    ...
    public AbstractCardModifier makeCopy() {
        return new FlawedModifier(isPermanent);
    }
}