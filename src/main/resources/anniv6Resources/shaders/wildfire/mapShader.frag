...
vec3 fireParticles(in vec2 uv, in vec2 originalUV)
{
    ...
    //Do not use fade in or fade out for map shader
...
void main()
{
    ...
    if (outputColor.a > 0.0) {
        vec2 uv = (2.0 * gl_FragCoord.xy - u_screenSize.xy) / u_screenSize.x;
        ...
        //Remove cutoff for map so smoke is visible at all times
